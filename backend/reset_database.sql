-- Run this in Supabase SQL Editor to reset and recreate the database
-- This drops existing tables and recreates them with the updated schema

-- Drop existing tables (cascade handles dependencies)
DROP TABLE IF EXISTS student_reasoning CASCADE;
DROP TABLE IF EXISTS otrs CASCADE;
DROP TABLE IF EXISTS sessions CASCADE;
DROP FUNCTION IF EXISTS get_session_metrics CASCADE;
...
-- OTRs table (removed CHECK constraints to allow flexible data)
CREATE TABLE otrs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
